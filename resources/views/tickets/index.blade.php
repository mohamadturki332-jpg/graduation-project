...
@section('title', __('Tickets') . ' — ' . config('app.name'))
...
<span class="text-primary">{{ __('Operational View') }}</span>
</div>
...
<p class="text-on-surface-variant font-light">{{ __('No tickets yet.') }}</p>
</div>
@else
...
@empty
<tr><td colspan="9" class="px-4 py-16 text-center italic text-on-surface-variant">{{ __('No tickets match your filters.') }}</td></tr>
@endforelse
</tbody>
</table>
</div>
...
<form method="GET" action="{{ route('tickets.index') }}" class="flex flex-col h-full">
...
<a href="{{ route('tickets.index') }}" class="text-[10px] text-primary font-bold hover:underline">{{ __('RESET') }}</a>
</div>
...
<script>
// Auto-refresh when a new ticket arrives: poll a tiny endpoint every 7s and
// reload the page when the max ticket id grows. The first response is the
// baseline, so opening the page never triggers a reload by itself.
(function () {
    let baseline = null;
